...
const countByCategory = () => {
    let count = 0;
    document.querySelector(".unread").innerHTML = "You have unreaded(by category): "
    if(JSON.parse(localStorage.getItem("categories")) !== null){
        JSON.parse(localStorage.getItem("categories")).forEach(category => {
            if(JSON.parse(localStorage.getItem("bookTable")) !== null){
                JSON.parse(localStorage.getItem("bookTable")).forEach(book => {
                    if(book.category === category)count++;
                })
                document.querySelector(".unread").innerHTML += `${count} ${category}, `;
                count = 0;
            }            
        });
    }
